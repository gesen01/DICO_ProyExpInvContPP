...
Fecha BETWEEN '01/12/2023' AND '31/12/2023'
AND Grupo='PP' order by id asc

--SELECT * FROM Axu

SELECT c.ID, c.Empresa, c.Mov, c.MovID, c.FechaContable, c.Estatus, d.Cuenta, 'Debe'=SUM(ISNULL(CASE WHEN c.Estatus='CANCELADO' THEN 0 ELSE d.Debe END,0)),'Haber'=SUM(ISNULL(CASE WHEN c.Estatus='CANCELADO' THEN 0 ELSE d.Haber END,0)), c.OrigenTipo,c.Origen, c.OrigenID
INTO #CONT 
FROM Cont c 
JOIN ContD d On c.ID=d.ID 
WHERE c.FechaContable BETWEEN '01/12/2023' AND '31/12/2023'
AND d.Cuenta='115-003-000'-- and isnull(Debe,0)>0 
--AND c.ID=73223
GROUP BY  c.ID, c.Empresa, c.Mov, c.MovID, c.FechaContable, c.Estatus, d.Cuenta, d.Debe, d.Haber, c.OrigenTipo,c.Origen, c.OrigenID
 --order by c.id asc
...
-- WHERE Clave='PROD.E'
 ...
 INTO #PROD 
 FROM #CONTPROD c JOIN ProdD d On c.IDModulo = d.ID
 JOIN Prod p On c.IDModulo=p.ID
 ...
 SELECt p.ID, p.Empresa, p.Mov, p.MovID, p.FechaContable, p.Estatus, p.Cuenta, p.Debe, p.Haber, p.OrigenTipo, p.IDmodulo, p.MovProd, p.MovIDProd, p.EstatusProd, p.Articulo, p.ProdSerieLote, p.Costo, P.Total , 'ModuloINV'= pslc.Modulo, 'ModuloIDINV'= pslc.ModuloID, 'DebeInv'=SUM(ISNULL(Abono,0)), 'HaberInv'=SUM(ISNULL(Cargo,0))
 INTO #ProdInv 
 ...
 SELECT p.ID, p.Empresa, p.Mov, p.MovID, p.FechaContable, p.Estatus, p.Cuenta, p.Debe, p.Haber, p.OrigenTipo, p.IDmodulo, p.MovProd, p.MovIDProd, p.EstatusProd, p.ModuloINV, p.ModuloIDINV,'DebeInv'=SUM(ISNULL(DebeInv,0)), 'HaberInv'=SUM(ISNULL(HaberInv,0))
 into #ProdInv2
